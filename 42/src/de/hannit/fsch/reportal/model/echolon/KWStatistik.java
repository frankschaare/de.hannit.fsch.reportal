...
public class KWStatistik extends EcholonStatistik
{
...
private Stream<Vorgang> si = null;
	...
	public KWStatistik(LocalDate incoming, KalenderWoche kalenderWoche) 
	{
	vorgaengeBerichtszeitraum = new ArrayList<Vorgang>();

	this.kw = kalenderWoche;	
	setLabel(kw.getLabel());
	}
	...
	public void setIndex(LocalDate incoming, int incomingKW)
	{
	String strJahr = Zeitraum.DF_JAHR.format(incoming);
	String strKW = incomingKW < 10 ? "0" + String.valueOf(incomingKW) : String.valueOf(incomingKW);  
	
	this.index = strJahr + strKW;
	}	
